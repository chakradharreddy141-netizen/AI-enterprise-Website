...
function initCanvas() {
  ...
  const ringMatCyan = new THREE.MeshBasicMaterial({ color: 0xec4899 });
  ...
  const lightCyan = new THREE.PointLight(0xec4899, 5, 20);
...
function initStatsCounters() {
  const statsSection = document.getElementById('sec-m5');
  
  const stats = [
    { id: "stat-count-1", target: 300, suffix: "+", decimals: 0 },
    { id: "stat-count-2", target: 450, suffix: "+ hrs", decimals: 0 },
    { id: "stat-count-3", target: 85000, suffix: "+", decimals: 0 },
    { id: "stat-count-4", target: 1200, suffix: " miles", decimals: 0 }
  ];
...
const stepsData = {
  1: {
    ...
    desc: "We created our very first joint playlist. Every song added became a secret code and a sweet confession of our feelings across the miles."
  },
  2: {
    title: "Cute Nicknames",
    desc: "From 'cutie' to specialized inside jokes, we developed our own language that instantly puts a smile on our faces."
  },
  3: {
    title: "Daily Video Dates",
    desc: "No matter how busy or tired we are, seeing each other's faces before sleeping is our favorite daily anchor."
  }
};
...
window.handleFormSubmit = function(e) {
  ...
  if (!btn) return;
  btn.innerText = "Sending letter to our love vault...";
  btn.disabled = true;

  setTimeout(() => {
    btn.innerText = "Love Letter Saved Forever ❤️";
    btn.className = "w-full transition-spring inline-flex items-center justify-center font-semibold bg-pink-500 text-white py-4 rounded-xl shadow-glow-cyan";
    alert("Happy 10th Month Anniversary! Your love letter has been successfully saved in our heart database.");
  }, 1500);
};
...
function initParticlesCanvas() {
  ...
  class Particle {
    ...
    draw() {
      ...
      if (this.size > 1.2) {
        ...
        const glowColor = this.color.includes('rgba') ? this.color.replace(/[\d\.]+\)$/, `${alpha})`) : 'rgba(236, 72, 153, 0.06)';
  ...
  function setupParticles() {
    ...
    for (let i = 0; i < numberOfParticles; i++) {
      ...
      let color = Math.random() > 0.5 ? 'rgba(236, 72, 153, 0.4)' : 'rgba(168, 85, 247, 0.3)';
    ...
    for (let a = 0; a < particlesArray.length; a++) {
      for (let b = a + 1; b < particlesArray.length; b++) {
        ...
        if (distance < maxDistanceSq) {
          ...
          particlesCtx.strokeStyle = `rgba(236, 72, 153, ${opacityValue * 0.12})`;
...
function updateActiveNavLink(progress) {
  ...
  if (progress >= 0.15 && progress <= 0.90) {
    // Divide the 0.15 to 0.90 range (length 0.75) into 9 equal sections of length 0.08333
    activeIndex = Math.floor((progress - 0.15) / 0.08333);
    activeIndex = Math.min(activeIndex, 8);
  }
  ...
  navLinks.forEach(link => {
    link.addEventListener('click', (e) => {
      ...
      if (targetSec && lenis) {
        ...
        const leaveVal = parseFloat(targetSec.dataset.leave) / 100;
        // Scroll exactly to the middle of the section's active range
        const midVal = enterVal + (leaveVal - enterVal) / 2;
        const maxScroll = scrollContainer.offsetHeight - window.innerHeight;
        const targetScroll = scrollContainer.offsetTop + (maxScroll * midVal);
